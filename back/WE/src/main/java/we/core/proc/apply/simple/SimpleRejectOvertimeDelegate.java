 ...
 */
@Service
public class SimpleRejectOvertimeDelegate implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		for (Task task : taskList) {
			...
			this.Reject(processInstanceId, task.getId(), assignee);
					...
					for (IdentityLink identityLink : identityLinkList) {
						if (StringUtils.hasText(identityLink.getGroupId())) {
							...
							for (String userId : searchIdList) {
								this.Reject(processInstanceId, task.getId(), userId);
							}
						}
					}
				}
		}
		execution.setVariable("approval_result", "1");
	}

	public void Reject(String processInstanceId, String taskId, String userId) throws Exception {
		// 申请实例ID
		if (StringUtils.hasText(taskId) == false) {
			throw new FacadeException("申请实例ID未输入。");
		}
		String commentValue = "超时自动拒绝";
		String status = "9";
		// 数据源信息
		...
		variables.put("approval_result", "1");
